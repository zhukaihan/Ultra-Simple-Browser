...
class ViewController: UIViewController, UIContentContainer, WKNavigationDelegate, UITextFieldDelegate, UIGestureRecognizerDelegate,  UITableViewDataSource, UITableViewDelegate {
    
    
    //////////Variables
    var homewebpage : String!    //the url of home page
    var statusbarpref = NSUserDefaults.standardUserDefaults().boolForKey("preferstatusbarhidden")    //see if user want to hide the status bar
    var defaultSearchEngine = NSUserDefaults.standardUserDefaults().stringForKey("searchtype")    //the default search enging user selected
    ...
    var undoWebView: WKWebView? = WKWebView()/Users/peterzhu/Ultra-Simple-Browserr
    var isLongPressed: Bool = false
    var backForwardListTableView: UITableView! = UITableView()
    ...
    var edgepanorigin: CGPoint = CGPointMake(0, 0)
    ...
    override func loadView() {
        ...
        let img = UIImage(named: "pages.png")
        webViewSwitch = UIBarButtonItem(image: img, style: .Plain, target: self, action: "showAllWebViews")
        ...
        edgeSwipe.addTarget(self, action: "hidetoolbar")
        ...
        homebutton = UIButton()
        homebutton.frame = CGRectMake(view.frame.width / 4 * 2 - 59, view.frame.height - 44 - 59, 59, 59)
        homebutton.setImage(homebuttonimg, forState: .Normal)
        
        let sharebuttonimg = UIImage(named: "share.png")
        sharebutton = UIButton()
        sharebutton.frame = CGRectMake(view.frame.width / 4 * 3 - 59, view.frame.height - 44 - 59, 59, 59)
        ...
        openinsafaributton.frame = CGRectMake(view.frame.width - 59, view.frame.height - 44 - 59, 59, 59)
        openinsafaributton.setImage(openinsafaributtonimg, forState: .Normal)
        
        textField.delegate = self
        
        ////load the last webpage that user visit last time
    ...
    func orientationDidChange(sender: NSNotification) {
        if ((UIDeviceOrientationIsLandscape(UIDevice.currentDevice().orientation) && !orientationchanged) || (UIDeviceOrientationIsPortrait(UIDevice.currentDevice().orientation) && orientationchanged)) {
            ...
            if (backForwardListTableView.window != nil) {
                backForwardListTableView.frame = CGRectMake(0, 0, view.frame.width, view.frame.height - 44)
            }
    ...
    */
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if backForwardListTableView.tag == 0 {
            let number = webViews[currentWebView]?.backForwardList.backList.count
            return number!
        } else if backForwardListTableView.tag == 1 {
    ...
    func showGoBackList(rec: UILongPressGestureRecognizer) {
        if rec.state == .Began {
            backForwardListTableView = UITableView()
            backForwardListTableView.dataSource = self
            backForwardListTableView.delegate = self
            backForwardListTableView.frame = CGRectMake(0, view.frame.height, view.frame.width, view.frame.height - 44)
            backForwardListTableView.tag = 0
            
            UIView.beginAnimations("animateWebView", context: nil)
            UIView.setAnimationDuration(0.1)
            
            backForwardListTableView.frame = CGRectMake(0, 0, view.frame.width, view.frame.height - 44)
            //backForwardListTableView.style = .Plain
            self.view.addSubview(backForwardListTableView)
            ...
            let cancelButton: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: .Cancel, target: self, action: "hideGoBackForwardList")
            var toolBarItems = [
                listLabelItem,
                flexibleSpaceBarButtonItem,
                cancelButton
    ...
    func showGoForwardList(rec: UILongPressGestureRecognizer) {
        if rec.state == .Began {
            ...
            backForwardListTableView.delegate = self
            backForwardListTableView.frame = CGRectMake(0, view.frame.height, view.frame.width, view.frame.height - 44)
            backForwardListTableView.tag = 1
    ...
    //to hide table view
    func hideGoBackForwardList() {
        UIView.beginAnimations("animateWebView", context: nil)
        UIView.setAnimationDuration(0.1)
        
        backForwardListTableView.removeFromSuperview()
        configtoolbaritems()
    ...
    func hidetoolbar() {
        ...
        case .Changed:
            ...
            edgepanorigin = edgeSwipe.locationInView(self.view)
            if ((edgepanorigin.x >= sharebutton.frame.minX) && (edgepanorigin.x <= sharebutton.frame.maxX)) && ((edgepanorigin.y >= sharebutton.frame.minY) && (edgepanorigin.y <= sharebutton.frame.maxY)) {
                sharebutton.backgroundImageForState(.Highlighted)
            } else {
                sharebutton.backgroundImageForState(.Normal)
            }
        case .Ended:
            if edgeSwipe.translationInView(toolBar).y > 0 {
                webViews[currentWebView]?.frame = CGRectMake(0, 0, view.frame.width, view.frame.height)
                progressBar.frame = CGRectMake(0, view.frame.height - 3, view.frame.width, 3)
                toolBar.hidden = true
            }
            edgepanorigin = edgeSwipe.locationInView(self.view)
            if ((edgepanorigin.x >= homebutton.frame.minX) && (edgepanorigin.x <= homebutton.frame.maxX)) && ((edgepanorigin.y >= homebutton.frame.minY) && (edgepanorigin.y <= homebutton.frame.maxY)) {
                goHome()
            } else if ((edgepanorigin.x >= sharebutton.frame.minX) && (edgepanorigin.x <= sharebutton.frame.maxX)) && ((edgepanorigin.y >= sharebutton.frame.minY) && (edgepanorigin.y <= sharebutton.frame.maxY)) {
                toShare()
            } else if ((edgepanorigin.x >= openinsafaributton.frame.minX) && (edgepanorigin.x <= openinsafaributton.frame.maxX)) && ((edgepanorigin.y >= openinsafaributton.frame.minY) && (edgepanorigin.y <= openinsafaributton.frame.maxY)) {
                toOpenInSafari()
            }
            homebutton.removeFromSuperview()
            sharebutton.removeFromSuperview()
            openinsafaributton.removeFromSuperview()
            UIView.commitAnimations()
        default: true
        }
        UIView.commitAnimations()
